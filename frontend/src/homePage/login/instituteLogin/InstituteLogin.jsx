...
const InstituteLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Send token to backend to fetch full institute profile
      const response = await fetch('https://soty-backend-n1b1.onrender.com/api/institute/login', {
        method: 'GET',
        headers: {
      ...
      if (data.success) {
        ...
        navigate('/instituteDashboard'); // Update to your actual route
      } else {
        setLoginError("Institute data not found");
  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl border border-gray-100">
        <div>
          <h2 className="mt-2 text-center text-3xl font-extrabold text-gray-900">Institute Login</h2>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          <div className="flex items-center justify-center mt-4">
            <span className="text-sm text-gray-600">
              ...
              <Link to="/register/institute" className="font-medium text-emerald-500 hover:text-emerald-600">
                Register here
              </Link>
            </span>
          </div>
